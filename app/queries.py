...
def aggregated_metrics(session, date):
    ...
    return (
        session.query(
            ...
            func.sum(OrderLines.full_price_amount)
...
def commission_by_promotion(session, date):
    ...
    return (
        ...
        .join(OrderLines, OrderLines.order_id == Orders.id)
        .join(Commissions, Orders.vendor_id == Commissions.vendor_id)
        .join(ProductPromotions, ProductPromotions.product_id == OrderLines.product_id)
        .group_by(ProductPromotions.promotion_id)
        .all()
    )
...
def total_commission(session, date):
    ...
    """
    return (
        session.query(Orders, func.sum(OrderLines.total_amount * Commissions.rate))
        .filter(Orders.created_at.contains(date), Commissions.date.contains(date))
        .join(OrderLines, Orders.id == OrderLines.order_id)
        .join(Commissions, Commissions.vendor_id == Orders.vendor_id)
        .first()[1]
    )
...
def get_data(session, date):
    ...
    commissions_dict["promotions"] = {
        key: val for _, key, val in commission_by_promotion(session, date)
    }
    commissions_dict["total"] = total_commission(session, date)
    commissions_dict["order_average"] = commissions_order_average(
        session, date, commissions_dict["total"]
    )
